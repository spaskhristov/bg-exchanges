...
public class HTMLRead {
    ...
    if (isPopularCurrency) {
      for (ExchangeInstitution institution : institutions) {
        try {
          ...
          } else if (institution.getId() == 20) {
            allTD = doc.getElementsByTag("span");
            institution.setSetCurrency(TDWorks.setCurrency(allTD, 33, 36, 6));
            continue;
          } else if (institution.getId() == 10) {
            allTD = doc.getElementsByClass("table-flex__col");
          }
  ...
  private static void fillFixingBNB() throws IOException, MalformedURLException {
